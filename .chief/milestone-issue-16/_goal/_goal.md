...
- **Block B — Builder API:** `TerraKit` with the full method set — `terraform`,
...
- **Block E — full example migration** beyond the single azurerm example; multi-file
  output; monorepo / separate `terrakit-codegen` package.
...
3. **Both emitters work.** Given a `TerraKit` program, `emitJson` yields valid Terraform
...
6. **Public API is the intended surface.** `src/index.ts` exports exactly `TerraKit`,
...
- Keep the existing build chain (`tsc → babel esm/cjs`) and the `terrakit` tsconfig path alias.
